...
These three control bytes contain the target address of the memory access and the type of operation (R/W).
...
The time from the falling edge until the transmission of the first command byte must be at least 15us.
...
Unfortunately, the interrupt vector table lives in the middle of this memory so it cannot be used contiguosly, but is separated into two regions.
...
The code/data for the firmware running on the MSP also has to be placed within this area.
...
So effectively, the memory area from 0x6000 to 0x24000, excluding the IVT (0xFF80-0xFFFF) is available as data storage.
...
Any operation extending over the IVT region or the end of the memory will get truncated.

**The code on the SPI controller talking to the NVM must avoid reading/writing to the IVT region, which lies between 0x9F80 and 0x9FFF from its view.**
